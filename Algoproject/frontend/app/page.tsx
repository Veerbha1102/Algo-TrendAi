      ...
      if (currentUser && typeof window !== "undefined") {
        try {
          ...
          if (permission === 'granted') {
            ...
            // A valid Firebase VAPID key is a Base64url-encoded 65-byte EC public key (~87-88 chars).
            // Skip silently if the key is missing or looks like a placeholder to avoid InvalidAccessError spam.
            if (vapidKey && vapidKey.length >= 80) {
              ...
              console.warn("FCM Init skipped: NEXT_PUBLIC_FIREBASE_VAPID_KEY looks invalid (too short). Get the correct key from Firebase Console → Project Settings → Cloud Messaging → Web Push certificates.");
            }
          } // end if (permission === 'granted')
        } catch(e) {
                   ...
                   <h2 className="text-3xl font-extrabold capitalize text-on-surface tracking-tight mb-8">
                     ...
                     {currentView === 'portfolio' && (
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                       ...
                       {/* AlgoKit Chain Health Monitor */}
                       <ChainHealthPanel address={wallet} />
                       </div>
                    )}
                   ...
                   {currentView === 'ai-brain' && (
                      <div className="space-y-6">
                        ...
                        <div className="flex items-center gap-4 mb-2">
                          <div className="w-14 h-14 rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center shadow-[0_0_20px_rgba(46,91,255,0.15)]">
                            <span className="material-symbols-outlined text-2xl text-primary" style={{fontVariationSettings:"'FILL' 1"}}>neurology</span>
                          </div>
                          <div>
                            <h3 className="text-2xl font-extrabold text-white tracking-tight">AI Brain Log</h3>
                            <p className="text-xs text-on-surface-variant mt-1 max-w-xl">Every decision the AI makes is fully transparent. See the exact indicators, data sources, and mathematical reasoning behind each trade — permanently preserved on Firestore.</p>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                          {[
                            { label: "Total Decisions", value: tradeHistory.length, color: "text-white" },
                            { label: "BUY Entries", value: tradeHistory.filter((t:any) => t.action === 'BUY').length, color: "text-secondary" },
                            { label: "SELL Exits", value: tradeHistory.filter((t:any) => t.action === 'SELL').length, color: "text-error" },
                            { label: "Avg. Confidence", value: tradeHistory.length > 0 ? `${(tradeHistory.reduce((acc:number, t:any) => acc + (t.ai_confidence || 0), 0) / tradeHistory.length * 100).toFixed(0)}%` : "N/A", color: "text-primary" },
                          ].map(stat => (
                            <div key={stat.label} className="bg-surface-container p-4 rounded-2xl border border-outline-variant/10">
                        ...
                        {/* Decision Ledger */}
                        {tradeHistory.length === 0 ? (
                          <div className="bg-surface-container rounded-2xl border border-outline-variant/10 p-16 text-center">
                            <span className="material-symbols-outlined text-5xl text-on-surface-variant/30 block mb-4" style={{fontVariationSettings:"'FILL' 1"}}>neurology</span>
                            <p className="text-on-surface-variant text-sm font-medium">No trade decisions yet.</p>
                            <p className="text-on-surface-variant/50 text-xs mt-1">The AI Brain Log will populate as the Autopilot executes its first trade.</p>
                          </div>
                        ) : (
                          <div className="space-y-4">
                            {tradeHistory.map((t: any) => {
                              const isBuy = t.action === 'BUY';
                              const conf = t.ai_confidence ? Math.round(t.ai_confidence * 100) : 0;
                              return (
                                <div key={t.id} className={`rounded-2xl border overflow-hidden shadow-lg ${isBuy ? 'border-secondary/20 bg-gradient-to-br from-[#0d1f18] to-[#0B0E14]' : 'border-error/20 bg-gradient-to-br from-[#1f0d0d] to-[#0B0E14]'}`}>
                                  {/* Top row */}
                                  <div className={`flex items-center justify-between px-5 py-3 border-b ${isBuy ? 'border-secondary/10 bg-secondary/5' : 'border-error/10 bg-error/5'}`}>
                                    <div className="flex items-center gap-3">
                                      <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${isBuy ? 'bg-secondary/15' : 'bg-error/15'}`}>
                                        <span className={`material-symbols-outlined text-base ${isBuy ? 'text-secondary' : 'text-error'}`} style={{fontVariationSettings:"'FILL' 1"}}>{isBuy ? 'trending_up' : 'trending_down'}</span>
                                      </div>
                                      <div>
                                        <p className={`text-sm font-extrabold tracking-wide ${isBuy ? 'text-secondary' : 'text-error'}`}>
                                          {isBuy ? '⬆ BUY EXECUTED' : '⬇ SELL EXECUTED'} — ALGO/USD
                                        </p>
                                        <p className="text-[10px] text-on-surface-variant font-mono">{t.timestamp?.toDate ? t.timestamp.toDate().toLocaleString() : 'Processing...'}</p>
                                      </div>
                                    </div>
                                    <div className="flex items-center gap-3">
                                      {/* Grade Badge */}
                                      {t.ai_grade && (
                                        <span className={`text-[10px] font-bold px-3 py-1.5 rounded-lg border tracking-widest ${isBuy ? 'bg-secondary/10 text-secondary border-secondary/20' : 'bg-error/10 text-error border-error/20'}`}>
                                          {t.ai_grade}
                                        </span>
                                      )}
                                      {/* Tx Link */}
                                      {t.tx_id && (
                                        <a href={`https://testnet.explorer.perawallet.app/tx/${t.tx_id}`} target="_blank" rel="noreferrer"
                                           className="text-primary text-[10px] font-mono hover:underline flex items-center gap-1 bg-primary/10 border border-primary/20 px-2 py-1.5 rounded-lg">
                                          {t.tx_id.slice(0,10)}... <span className="material-symbols-outlined text-[12px]">open_in_new</span>
                                        </a>
                                      )}
                                    </div>
                                  </div>
                                  ...
                                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-0 divide-y lg:divide-y-0 lg:divide-x divide-white/5">
                                    ...
                                    <div className="p-5 flex flex-col gap-3">
                                      <p className="text-[10px] uppercase tracking-widest text-on-surface-variant font-bold flex items-center gap-1.5">
                                        <span className="material-symbols-outlined text-[14px]">bar_chart</span> Confidence Score
                                      </p>
                                      <div className="flex items-end gap-2">
                                        <p className={`text-4xl font-mono font-black ${conf >= 70 ? 'text-secondary' : conf >= 40 ? 'text-primary' : 'text-error'}`}>{conf}%</p>
                                      </div>
                                      <div className="w-full bg-black/30 rounded-full h-2">
                                        <div
                                          className={`h-full rounded-full transition-all duration-700 ${conf >= 70 ? 'bg-secondary' : conf >= 40 ? 'bg-primary' : 'bg-error'}`}
                                          style={{width: `${conf}%`}}
                                        />
                                      </div>
                                      <div className="mt-1">
                                        <p className="text-[10px] text-on-surface-variant font-bold uppercase tracking-widest">Trade Amount</p>
                                        <p className="font-mono font-bold text-white text-sm mt-0.5">{t.amount} ALGO</p>
                                      </div>
                                    </div>
                                    ...
                                    <div className="p-5 flex flex-col gap-3">
                                      <p className="text-[10px] uppercase tracking-widest text-on-surface-variant font-bold flex items-center gap-1.5">
                                        <span className="material-symbols-outlined text-[14px]">database</span> Data Sources
                                      </p>
                                      <div className="space-y-2 text-xs">
                                        <div className="flex items-center justify-between bg-black/20 rounded-lg px-3 py-2 border border-white/5">
                                          <span className="text-on-surface-variant">Price Feed</span>
                                          <span className="font-mono font-bold text-white">Yahoo Finance / yfinance</span>
                                        </div>
                                        <div className="flex items-center justify-between bg-black/20 rounded-lg px-3 py-2 border border-white/5">
                                          <span className="text-on-surface-variant">Indicators</span>
                                          <span className="font-mono font-bold text-white">pandas-ta (15m bars)</span>
                                        </div>
                                        <div className="flex items-center justify-between bg-black/20 rounded-lg px-3 py-2 border border-white/5">
                                          <span className="text-on-surface-variant">Macro Context</span>
                                          <span className="font-mono font-bold text-white">NIFTY 50 / ^NSEI</span>
                                        </div>
                                        <div className="flex items-center justify-between bg-black/20 rounded-lg px-3 py-2 border border-white/5">
                                          <span className="text-on-surface-variant">AI Model</span>
                                          <span className="font-mono font-bold text-primary">Gemini 2.5 Pro</span>
                                        </div>
                                        <div className="flex items-center justify-between bg-black/20 rounded-lg px-3 py-2 border border-white/5">
                                          <span className="text-on-surface-variant">Blockchain</span>
                                          <span className="font-mono font-bold text-secondary">Algorand Testnet</span>
                                        </div>
                                      </div>
                                    </div>
                                    ...
                                    <div className="p-5 flex flex-col gap-3">
                                      ...
                                      {t.ai_explanation ? (
                                        <div className="bg-black/30 rounded-xl p-4 border border-white/5 flex-1">
                                          <p className="text-xs text-on-surface-variant/90 leading-relaxed font-mono">
                                            {t.ai_explanation}
                                          </p>
                                        </div>
                                      ) : (
                                        <div className="bg-black/20 rounded-xl p-4 border border-white/5 flex-1 flex items-center justify-center">
                                          <p className="text-xs text-on-surface-variant/40 italic">Reasoning not captured — predates this AI upgrade.</p>
                                        </div>
                                      )}
                                    </div>
                                  </div>
